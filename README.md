...
Enable the recovery units you need:

```bash
sudo systemctl enable --now elan-guardian-resume.service elan-guardian-watch.service
```

Arch Linux and compatible Arch-based distributions are the supported packaging
and maintenance targets. If a kernel update cannot build the optional module,
the userspace recovery paths remain available with the distribution driver.
